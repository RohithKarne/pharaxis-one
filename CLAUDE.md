...
7. **No file is created at the top level of the repository.** Every new file goes inside an existing folder — `apps`, `docs`, `.github`, `.claude`, `.githooks`, `ops`, `scripts`. Where none fits, agree the new folder with Rohith first. **Only four files live at the root** — `.gitignore`, `CLAUDE.md`, `README.md`, `LICENSE` — each pinned there by a tool that looks nowhere else, and nothing joins that list without Rohith's word. → SOP §38.11
