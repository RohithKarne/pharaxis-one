...
router.get('/:clientId', authenticateAdmin, requireClientAccess, (req, res) => {
  ...
  const total = db.prepare('SELECT COUNT(*) as n FROM cp_submissions WHERE client_id = ?').get(req.params.clientId);

  res.json({ submissions: rows, counts, total: total?.n || 0 });
});

// PATCH /api/admin/submissions/:clientId/:submissionId — update status
router.patch('/:clientId/:submissionId', authenticateAdmin, requireClientAccess, (req, res) => {
  ...
  db.prepare(`
    UPDATE cp_submissions SET status = ?, updated_at = datetime('now')
    WHERE id = ? AND client_id = ?
  `).run(status, req.params.submissionId, req.params.clientId);
