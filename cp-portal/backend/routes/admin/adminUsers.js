...
router.get('/:clientId', authenticateAdmin, requireClientAccess, requireRole('superadmin', 'admin'), (req, res) => {
...
router.post('/:clientId', authenticateAdmin, requireClientAccess, requireRole('superadmin', 'admin'), (req, res) => {
  const clientId = parseInt(req.params.clientId);
  ...
  const existing = db.prepare('SELECT id FROM cp_admin_users WHERE email = ?').get(email);
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `).run(name, email.toLowerCase().trim(), hash, role, clientId);

  audit(req.admin, clientId, 'create', 'admin_user', result.lastInsertRowid, `Created ${role}: ${email}`);

  res.status(201).json({
    user: { id: result.lastInsertRowid, name, email: email.toLowerCase().trim(), role, client_id: clientId, is_active: 1 }
  });
});

// ── PATCH /:clientId/:userId — update name, role, or is_active ───────────────
router.patch('/:clientId/:userId', authenticateAdmin, requireClientAccess, requireRole('superadmin', 'admin'), (req, res) => {
  const clientId = parseInt(req.params.clientId);
  const userId   = parseInt(req.params.userId);
  const { name, role, is_active } = req.body;

  const user = db.prepare('SELECT * FROM cp_admin_users WHERE id = ? AND client_id = ?').get(userId, clientId);
  ...
  db.prepare(`
    UPDATE cp_admin_users SET name = ?, role = ?, is_active = ?, updated_at = datetime('now') WHERE id = ?
  `).run(newName, newRole, newIsActive, userId);

  audit(req.admin, clientId, 'update', 'admin_user', userId, `Updated ${user.email}: role=${newRole}, active=${newIsActive}`);

  res.json({ message: 'Admin user updated.' });
});

// ── DELETE /:clientId/:userId — deactivate (soft delete) ─────────────────────
router.delete('/:clientId/:userId', authenticateAdmin, requireClientAccess, requireRole('superadmin', 'admin'), (req, res) => {
  const clientId = parseInt(req.params.clientId);
  const userId   = parseInt(req.params.userId);
  ...
  db.prepare(`UPDATE cp_admin_users SET is_active = 0, updated_at = datetime('now') WHERE id = ?`).run(userId);
  audit(req.admin, clientId, 'deactivate', 'admin_user', userId, `Deactivated ${user.email}`);
