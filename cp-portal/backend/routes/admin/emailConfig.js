...
router.patch('/:clientId', authenticateAdmin, requireClientAccess, requireRole('superadmin', 'admin'), (req, res) => {
  const { clientId } = req.params
  const { smtp_host, smtp_port, smtp_encryption, smtp_username, smtp_password, from_email, from_name, is_active } = req.body

  const existing = db.prepare('SELECT id FROM cp_email_config WHERE client_id = ?').get(clientId)
  ...
  if (existing) {
    ...
    db.prepare(`UPDATE cp_email_config SET ${updates.join(', ')} WHERE client_id = ?`).run(...params)
  } else {
    db.prepare(`
  ...
  audit(req.admin, clientId, 'UPDATE', 'email_config', Number(clientId), { smtp_host, smtp_username, is_active })
...
router.post('/:clientId/test', authenticateAdmin, requireClientAccess, requireRole('superadmin', 'admin'), async (req, res) => {
  ...
  try {
    ...
    audit(req.admin, clientId, 'TEST_EMAIL', 'email_config', Number(clientId), { to })
