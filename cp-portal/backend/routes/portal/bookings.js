...
router.post('/:clientCode/:mslId', authenticatePortal, (req, res) => {
  ...
  const msl = db.prepare('SELECT id FROM cp_msls WHERE id = ? AND client_id = ? AND is_active = 1').get(mslId, client.id);
  ...
  `).get(mslId, requester_email.toLowerCase().trim());
...
router.get('/:clientCode', authenticatePortal, (req, res) => {
  ...
  `).all(client.id, req.portalUser.id);
