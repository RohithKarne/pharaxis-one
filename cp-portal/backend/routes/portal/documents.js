...
router.get('/', authenticatePortal, requirePortalAuth, (req, res) => {
  ...
  const docs = db.prepare(`
    ...
    WHERE client_id = ? AND is_active = 1
      AND (status = 'published' OR (status = 'scheduled' AND publish_at <= datetime('now')))
...
router.get('/:docId/download', authenticatePortal, requirePortalAuth, (req, res) => {
  ...
  db.prepare(`UPDATE cp_documents SET download_count = download_count + 1, updated_at = datetime('now') WHERE id = ?`).run(doc.id);
