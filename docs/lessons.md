...
## L-014 — Described an outcome I had not seen as what would happen (2026-09-22)
**What:** After writing a temporary password into the MIMS database, Rohith was told "MIMS should take you straight to a 'set a new password' screen." It did not — the reset-required sign-in path never recorded the session, so the middleware treated the token as revoked and bounced him to login every time. The message did say the author had not signed in, but in a separate sentence, and the claim itself was written in the future tense as fact. Cost: a second lost evening for the CEO, a second direct database write, and a fix that is committed but still not verified on screen. Postmortem: `docs/postmortems/2026-09-22-told-the-ceo-he-would-land-on-the-reset-screen.md`.
**Why:** The persona cannot enter passwords, so the path could not be exercised. Reading the code gave every signal that it worked, and the gap between "reads correctly" and "seen working" was filled with the expected outcome stated as the actual one. With §29 retired, nothing automated stood between that sentence and the CEO.
**Rule:** When you cannot exercise a path yourself, say what you expect **and** that you have not seen it — never describe an unverified outcome in the future tense as fact. The sentence that states what was NOT checked sits next to the claim it limits, in the same breath, not in a separate paragraph. "I expect X; I have not seen it" is always allowed. "X will happen" is allowed only after X has been seen on the real screen.
