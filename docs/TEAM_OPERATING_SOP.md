...
| `docs/` | Every document, including the change log and the postmortems. **Grouped by product from 2026-09-26** — `docs/mims/` and `docs/cp-portal/` hold what belongs to one product; anything shared stays at the top of `docs/`. This is grouping inside one documentation home, **not** a split back into the apps: the five per-app SOP files were absorbed here on 2026-08-07 precisely because people could not find things, and moving product documents under `apps/` would walk back into that |
...
> Seventh cloud routine. Governance lives here; the operational prompt lives in `docs/cp-portal/CP_PM.md`.
...
| Prompt | `docs/cp-portal/CP_PM.md` |
...
**The analyst-facing ticket is at most 15,000 characters** — description **9,500**, worked-examples comment **5,500**. The evidence comment (**800**, a bare citation list) sits outside it because the analyst never reads it. The epic is capped at **2,000**. **These ceilings override every other document, including `docs/cp-portal/CP_PM.md`.**
...
The full table is in `docs/cp-portal/CP_PM.md` §6. Three things about it belong here as governance rather than content:
...
5. **Never reveal the answer** for the live scenario. The worked examples in `docs/cp-portal/CP_PM.md` §9 deliberately use a different scenario, and that scenario is never issued as a live ticket.
