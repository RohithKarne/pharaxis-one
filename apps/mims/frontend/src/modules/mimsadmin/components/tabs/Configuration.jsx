import { findConfigLabel } from '../configItems'

export default function Configuration({ selectedItem }) {
  return (
      ...
      {!selectedItem ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60%', color: 'var(--text-muted)', gap: 12 }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
            Hover over <strong>Configuration</strong> in the top nav and select an item.
          </div>
        </div>
      ) : (
        <div style={{ maxWidth: 560 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '32px 28px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              Configuration for <strong>{findConfigLabel(selectedItem)}</strong> is coming soon.
            </div>
          </div>
        </div>
      )}
