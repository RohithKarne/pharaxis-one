...
export default function HelpGuide() {
  ...
  async function load() {
    ...
    try {
      ...
      const data = await httpFetch(`/api/admin/help?${params}`, { headers: H }).then(r => r.json())
  ...
  async function loadAttention() {
    const [staleData, covData] = await Promise.all([
      httpFetch('/api/admin/help/stale', { headers: H }).then(r => r.json()),
      httpFetch('/api/admin/help/coverage', { headers: H }).then(r => r.json()),
    ])
  ...
  async function softDelete(id) {
    ...
    await httpFetch(`/api/admin/help/${id}`, { method: 'DELETE', headers: H })
    toast.success('Article deactivated.')
    load()
  }

  async function markReviewed(id) {
    await httpFetch(`/api/admin/help/${id}/reviewed`, { method: 'PATCH', headers: H })
          ...
          <div style={{ background: '#fff', borderRadius: 12, width: 680, maxWidth: '95vw', padding: 28, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', marginBottom: 40 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {AUDIENCE_OPTIONS.map(opt => {
                  ...
                  const sel = aud.includes(opt.value)
                  return (
                    <button key={opt.value} type="button" onClick={() => toggleAudience(opt.value)} style={{
                      ...
                      border: `1px solid ${sel ? '#1d4ed8' : '#cbd5e1'}`,
                    }}>{opt.label}</button>
                  )
                })}
              </div>
            ...
            <div style={{ display: 'flex', gap: 14, marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 22 }}>
                ...
                <label htmlFor="hg-active" style={{ fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Active</label>
              </div>
            </div>
