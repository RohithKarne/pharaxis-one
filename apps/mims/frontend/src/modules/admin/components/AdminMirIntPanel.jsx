...
export default function AdminMirIntPanel({ config, setConfig, status, H }) {
  ...
  async function save() {
    setSaving(true)
    try {
      const res = await httpFetch('/api/admin/integrations/mir/config', { method: 'PUT', headers: H, body: JSON.stringify({ config }) })
      if (!res.ok) { alert('Failed to save MIR settings.'); return }  // WP7: surface save failures
    } catch { alert('Failed to save MIR settings.') } finally { setSaving(false) }
  }
