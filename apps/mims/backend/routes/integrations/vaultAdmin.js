...
router.get('/superadmin/vault-config', authenticate, requireRole('superadmin'), async (_req, res) => {
  try {
...
router.post('/superadmin/vault-config', authenticate, requireRole('superadmin'), async (req, res) => {
  try {
    const {
      org_id,
      vault_domain,
...
router.put('/superadmin/vault-config/:id', authenticate, requireRole('superadmin'), async (req, res) => {
  try {
    const { id } = req.params;
    const {
      vault_domain,
...
router.delete('/superadmin/vault-config/:id', authenticate, requireRole('superadmin'), async (req, res) => {
  try {
    const { id } = req.params;
