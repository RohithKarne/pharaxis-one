...
const ADMIN_ENTITIES = ['access_config', 'picklist', 'workflow', 'system_param', 'case_form_rule', 'security_group', 'user_role'];

// GET /api/admin/cm-audit-trail/admin-activity
router.get('/cm-audit-trail/admin-activity', authenticate, requireRole('admin', 'platform_admin'), async (req, res) => {
  try {
    const { entity, entity_id, action, user_id, from_date, to_date, page = 1, limit = 50 } = req.query;
    const lim    = Math.max(1, parseInt(limit, 10) || 50);
    const offset = (Math.max(1, parseInt(page, 10)) - 1) * lim;

    const entityList = entity ? [entity] : ADMIN_ENTITIES;
    const placeholders = entityList.map(() => '?').join(',');
    ...
    const [entries] = await pool.execute(
      `SELECT al.id, al.user_id, al.user_name, al.action, al.entity, al.entity_id,
              al.details, al.created_at, al.before_value AS old_value, al.after_value AS new_value, al.change_reason
       ${fromWhere}
       ORDER BY al.created_at DESC
       LIMIT ${lim} OFFSET ${offset}`,
      params
    ...
    console.error('GET /cm-audit-trail/admin-activity error:', err);
    res.status(500).json({ error: err.message });
  }
});
