...
router.get('/mi/tabs/:tabId/classification', authenticate, async (req, res) => {
  try {
    ...
    const params = globalScope ? [req.params.tabId] : [req.params.tabId, req.user.orgId];
  ...
  try {
    ...
    const [result] = await pool.execute(
      `UPDATE case_mi t
        ...
        WHERE t.id = ? AND ${orgClause}`,
      globalScope
        ? [assigned_to || null, req.params.tabId]
  ...
  try {
    ...
    const [result] = await pool.execute(
      `UPDATE case_mi t
        ...
        WHERE t.id = ? AND ${orgClause}`,
      globalScope
        ? [decision, req.user.userId, notes || null, req.params.tabId]
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT t.id AS mi_tab_id, t.case_id, t.is_off_label, t.off_label_indication,
        ...
        WHERE ${orgClause} AND t.promo_review_status = 'pending'
        ORDER BY t.promo_review_requested_at ASC
        LIMIT 200`,
      globalScope ? [] : [req.user.orgId]
    );
