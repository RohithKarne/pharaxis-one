...
async function orgForCase(caseId, req) { const row = await service.verifyCase(req.user.orgId, caseId); if (row) return row.org_id; if (req.user.role === 'superadmin') { const [[c]] = await pool.execute('SELECT org_id FROM cases WHERE id=?', [caseId]); return c?.org_id || null; } return null; }
