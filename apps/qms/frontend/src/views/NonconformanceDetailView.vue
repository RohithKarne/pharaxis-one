...
const availableActions = computed(() => {
  const status = nc.value?.status
  const id = route.params.id
  if (!status) return []
  const next = { Open: 'Containment', Containment: 'Dispositioned', Dispositioned: 'Closed' }
  if (next[status]) return [{ label: `Advance to ${next[status].replace(/([A-Z])/g, ' $1').trim()}`, endpoint: `/nonconformance/${id}`, method: 'PATCH', bodyFn: () => ({ status: next[status] }) }]
  return []
})
...
const nc = computed(() => detail.value?.nonconformance || null)
...
const currentPosition = computed(() => {
  ...
  const idx = allRecords.value.findIndex(r => r.id === route.params.id)
...
async function loadDetail() {
  ...
  error.value = ''
  try {
    const data = await apiRequest(`/nonconformance/${route.params.id}`)
    detail.value = data
  } catch (err) {
    error.value = err.message || 'Failed to load nonconformance record.'
  } finally {
    loading.value = false
  }
}
...
async function loadList() {
  try {
    ...
    allRecords.value = data.nonconformances || []
  } catch {
    allRecords.value = []
  }
}
...
function navigateRecord(delta) {
  const idx = allRecords.value.findIndex(r => r.id === route.params.id)
  const next = allRecords.value[idx + delta]
  if (next) router.push(`/nonconformance/${next.id}`)
}
...
<template>
  <div class="space-y-4">
    ...
    <template v-else-if="nc">

      <RecordHeader
        :breadcrumb="`Nonconformance / ${nc.nc_code}`"
        :doc-number="nc.nc_code"
        :title="nc.summary"
        :status="nc.status"
        :position="currentPosition"
        :total="allRecords.length || null"
        :lifecycle-states="NC_LIFECYCLE"
        @back="router.push('/nonconformance')"
      ...
      <div v-if="showActionPanel" class="rounded-xl border border-indigo-200 bg-indigo-50 p-4 space-y-3">
        <div class="flex items-center justify-between">
          <p class="text-sm font-semibold text-indigo-800">Lifecycle Actions — Current Status: <span class="font-bold">{{ nc.status }}</span></p>
          ...
          <div class="rounded-xl border border-slate-200 bg-white p-5">
            <dl class="grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-3">
              <div>
                ...
                <dd class="mt-0.5 font-medium text-slate-800">{{ nc.source_type || '—' }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Severity</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ nc.severity || '—' }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Disposition</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ nc.disposition || '—' }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Item Reference</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ nc.item_reference || '—' }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Due Date</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ formatDate(nc.due_date) }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">CAPA Links</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ capaLinks.length }}</dd>
              </div>
            </dl>
            <div v-if="nc.details" class="mt-4 border-t border-slate-100 pt-3">
              <dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Details</dt>
              <dd class="mt-1 text-sm leading-relaxed text-slate-700">{{ nc.details }}</dd>
            </div>
          </div>
