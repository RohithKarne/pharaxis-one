...
async function createComplaint() {
  ...
  try {
    ...
    message.value = 'Complaint created.';
...
const STATUS_COLORS = {
  ...
  CapaLinked: 'bg-purple-50 text-purple-700 border-purple-200',
  ...
  <div class="flex h-full flex-col">
    ...
    <div class="flex items-start justify-between rounded-xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
      <div>
        ...
        <h1 class="mt-0.5 text-2xl font-bold text-slate-900">Complaints</h1>
        <p class="mt-1 text-sm text-slate-500">Capture and investigate customer and regulatory complaints.</p>
      </div>
      <button
    ...
    <div class="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div class="border-b border-slate-100 bg-slate-50 px-4 py-3">
        <p class="text-sm font-semibold text-slate-700">
          {{ loading ? 'Loading…' : `${complaints.length} record${complaints.length !== 1 ? 's' : ''}` }}
        </p>
      </div>
      ...
      <div v-else-if="!complaints.length" class="py-16 text-center text-sm text-slate-400">
        No complaints found. Click <strong>New Complaint</strong> to get started.
      </div>
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody>
          <tr
            ...
            @click="router.push('/complaints/' + item.id)"
          >
            <td class="px-4 py-3 font-mono text-xs font-semibold text-indigo-600">{{ item.complaint_code }}</td>
            <td class="max-w-xs px-4 py-3 font-medium text-slate-800">
              <span class="line-clamp-1">{{ item.summary }}</span>
            </td>
            <td class="px-4 py-3 text-slate-600">{{ item.source_channel }}</td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-slate-600">{{ item.customer_name || '—' }}</td>
    ...
    <Transition enter-active-class="transition-transform duration-300" enter-from-class="translate-x-full" leave-active-class="transition-transform duration-300" leave-to-class="translate-x-full">
      <div v-if="showCreate" class="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 class="text-lg font-semibold text-slate-900">New Complaint</h2>
          <button class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600" @click="showCreate = false">
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5">
          <fieldset :disabled="isWriteDisabled" class="grid gap-3">
            <div>
              <label class="text-xs font-semibold text-slate-500">Source Channel</label>
              <select v-model="createForm.sourceChannel" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none">
            ...
            <div>
              ...
              <input v-model="createForm.summary" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none" placeholder="Brief complaint summary" />
            </div>
            <div>
            ...
            <div>
              ...
              <input v-model="createForm.customerName" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none" placeholder="Customer name" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                <label class="text-xs font-semibold text-slate-500">Batch / Lot</label>
                <input v-model="createForm.batchLotNo" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none" placeholder="Batch/Lot" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
