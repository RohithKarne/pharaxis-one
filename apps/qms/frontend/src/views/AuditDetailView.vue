...
const availableActions = computed(() => {
  ...
  if (!status) return []
  const actions = {
    Planned: [{ label: 'Start Audit', endpoint: `/audits/${id}/start`, method: 'POST', bodyFn: () => ({}), requiresNotes: false }],
    InProgress: [{ label: 'Close Audit', endpoint: `/audits/${id}/close`, method: 'POST', bodyFn: (notes) => ({ closureSummary: notes || 'Audit closed' }), requiresNotes: true, notesLabel: 'Closure summary (required)' }]
  }
...
const responses = computed(() => detail.value?.responses || [])
...
const AUDIT_LIFECYCLE = [
  ...
  { key: 'FindingsCaptured', label: 'Findings Captured' },
  { key: 'ResponseInProgress', label: 'Response In Progress' },
...
const sidebarSections = computed(() => [
  {
    ...
    items: [
      { key: 'findings', label: 'Findings', count: findings.value.length },
      { key: 'responses', label: 'Auditee Responses', count: responses.value.length }
    ]
  },
  {
    title: 'Actions',
...
const currentPosition = computed(() => {
  if (!allAudits.value.length) return null
  const idx = allAudits.value.findIndex(a => a.id === route.params.id)
...
async function loadDetail() {
  ...
  error.value = ''
  try {
    const data = await apiRequest(`/audits/${route.params.id}`)
    detail.value = data
  } catch (err) {
    error.value = err.message || 'Failed to load audit.'
  } finally {
    loading.value = false
  }
}
...
function navigateRecord(delta) {
  const idx = allAudits.value.findIndex(a => a.id === route.params.id)
  const next = allAudits.value[idx + delta]
  if (next) router.push(`/audits/${next.id}`)
}
...
<template>
  <div class="space-y-4">
    ...
    <template v-else-if="audit">
      ...
      <RecordHeader
        :breadcrumb="`Audits / ${audit.audit_code}`"
        :doc-number="audit.audit_code"
        :title="audit.audit_title"
        :status="audit.status"
        :position="currentPosition"
        :total="allAudits.length || null"
        :lifecycle-states="AUDIT_LIFECYCLE"
        @back="router.push('/audits')"
      ...
      <div v-if="showActionPanel" class="rounded-xl border border-indigo-200 bg-indigo-50 p-4 space-y-3">
        <div class="flex items-center justify-between">
          <p class="text-sm font-semibold text-indigo-800">Lifecycle Actions — Current Status: <span class="font-bold">{{ audit.status }}</span></p>
        ...
        <div class="flex-1 min-w-0 space-y-3">
          ...
          <div class="rounded-xl border border-slate-200 bg-white p-5">
            <dl class="grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-3">
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Audit Type</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ audit.audit_type || '—' }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Scope</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ audit.scope || '—' }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Planned Date</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ formatDate(audit.planned_date) }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Started</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ formatDate(audit.started_at) }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Actual End Date</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ formatDate(audit.actual_end_date) }}</dd>
              </div>
              <div>
                <dt class="text-xs font-medium text-slate-400 uppercase tracking-wide">Total Findings</dt>
                <dd class="mt-0.5 font-medium text-slate-800">{{ findings.length }}</dd>
              </div>
            </dl>
            <div v-if="audit.closure_summary" class="mt-4 border-t border-slate-100 pt-3">
              <dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Closure Summary</dt>
              <dd class="mt-1 text-sm leading-relaxed text-slate-700">{{ audit.closure_summary }}</dd>
            </div>
          </div>
          ...
          <div :id="`section-responses`">
            <RelatedRecordsPanel
              ...
              :columns="[
                ...
                { key: 'responded_at', label: 'Submitted' }
              ]"
              @create="() => {}"
            >
              <template #cell-responded_at="{ value }">{{ formatDate(value) }}</template>
            </RelatedRecordsPanel>
          </div>
