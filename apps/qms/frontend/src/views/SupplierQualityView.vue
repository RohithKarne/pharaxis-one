...
async function createSupplier() {
  if (!withRoles(['qa_reviewer', 'admin', 'superadmin'], (t) => { message.value = t; })) return;
  creating.value = true;
  try {
    await apiRequest('/supplier-quality/suppliers', { method: 'POST', body: supplierForm.value });
    message.value = 'Supplier created.';
  ...
  <div class="flex h-full flex-col">
    ...
    <div class="flex items-start justify-between rounded-xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
      <div>
        ...
        <h1 class="mt-0.5 text-2xl font-bold text-slate-900">Supplier Quality</h1>
        <p class="mt-1 text-sm text-slate-500">Manage supplier qualification, audits, and corrective action requests.</p>
      </div>
      <button
    ...
    <div class="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div v-else-if="!suppliers.length" class="py-16 text-center text-sm text-slate-400">
        No suppliers found. Click <strong>New Supplier</strong> to get started.
      </div>
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody>
          <tr
            ...
            @click="router.push('/supplier-quality/' + supplier.id)"
          >
            <td class="px-4 py-3 font-mono text-xs font-semibold text-indigo-600">{{ supplier.supplier_code }}</td>
            <td class="px-4 py-3 font-medium text-slate-800">{{ supplier.supplier_name }}</td>
            <td class="px-4 py-3 text-slate-600">{{ supplier.supplier_type }}</td>
            <td class="px-4 py-3">
              <span v-if="supplier.risk_level" class="rounded-full px-2 py-0.5 text-xs font-medium" :class="RISK_COLORS[supplier.risk_level] || 'bg-slate-100 text-slate-600'">{{ supplier.risk_level }}</span>
              <span v-else class="text-slate-400">—</span>
            </td>
            <td class="px-4 py-3 text-slate-600">{{ supplier.contact_email || '—' }}</td>
            <td class="px-4 py-3 text-slate-600">{{ formatDate(supplier.approved_at) }}</td>
            <td class="px-4 py-3">
              <span class="rounded-full border px-2 py-0.5 text-xs font-medium" :class="QUAL_COLORS[supplier.qualification_status] || 'bg-slate-50 text-slate-600 border-slate-200'">{{ supplier.qualification_status }}</span>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <Transition enter-active-class="transition-transform duration-300" enter-from-class="translate-x-full" leave-active-class="transition-transform duration-300" leave-to-class="translate-x-full">
      <div v-if="showCreate" class="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 class="text-lg font-semibold text-slate-900">New Supplier</h2>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5">
          <fieldset :disabled="isWriteDisabled" class="grid gap-3">
            <div>
              <label class="text-xs font-semibold text-slate-500">Supplier Name <span class="text-red-500">*</span></label>
              <input v-model="supplierForm.supplierName" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none" placeholder="Supplier name" />
            </div>
            <div>
              <label class="text-xs font-semibold text-slate-500">Supplier Type</label>
              <select v-model="supplierForm.supplierType" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none">
            ...
            <div>
              ...
              <input v-model="supplierForm.contactEmail" type="email" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none" placeholder="contact@supplier.com" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-xs font-semibold text-slate-500">Risk Level</label>
                <select v-model="supplierForm.riskLevel" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none">
                  <option>Low</option><option>Medium</option><option>High</option><option>Critical</option>
                </select>
              </div>
              <div>
                <label class="text-xs font-semibold text-slate-500">Initial Status</label>
                <select v-model="supplierForm.qualificationStatus" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none">
        ...
        <div class="border-t border-slate-200 px-6 py-4 flex gap-3">
          <button
            ...
            :disabled="!supplierForm.supplierName || creating || isWriteDisabled"
