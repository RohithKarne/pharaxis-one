...
router.post('/products-full', authenticate, requireRole('admin', 'superadmin'), requireOrg, async (req, res) => {
  try {
    const { trade_name, family_id, dosage, atc_code, is_active } = req.body;
    ...
    try {
      [result] = await pool.execute(
        'INSERT INTO products (trade_name, org_id, family_id, dosage, atc_code, is_active) VALUES (?, ?, ?, ?, ?, ?)',
        [trade_name.trim(), orgId, family_id || null, dosage || null, atc_code || null, is_active !== undefined ? (is_active ? 1 : 0) : 1]
      );
    } catch (colErr) {
    ...
    await audit(req.user.userId, req.user.email, 'CREATE', 'product', result.insertId, { trade_name, org_id: orgId, family_id });
...
router.put('/products-full/:id', authenticate, requireRole('admin', 'superadmin'), requireOrg, async (req, res) => {
  try {
    ...
    try {
      await pool.execute(
        'UPDATE products SET trade_name = ?, org_id = ?, family_id = ?, dosage = ?, atc_code = ?, is_active = ? WHERE id = ?',
        [trade_name, orgId, family_id || null, dosage || null, atc_code || null, is_active ? 1 : 0, id]
      );
    } catch (colErr) {
      if (colErr.code === 'ER_BAD_FIELD_ERROR') {
        await pool.execute(
    ...
    await audit(req.user.userId, req.user.email, 'UPDATE', 'product', Number(id), { trade_name, org_id: orgId, family_id, is_active });
