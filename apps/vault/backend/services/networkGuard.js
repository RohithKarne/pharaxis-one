...
function isAllowedHostname(hostname) {
  const allowlist = String(process.env.VAULT_OUTBOUND_ALLOWED_HOSTS || '')
    .split(',')
...
async function assertSafeOutboundUrl(rawUrl) {
  ...
  if (!['https:', 'http:'].includes(parsed.protocol)) throw new Error('Only http/https outbound URLs are allowed')
  if (parsed.username || parsed.password) throw new Error('Credential-in-URL is not allowed')

  const hostname = parsed.hostname.toLowerCase()
  if (hostname === 'localhost' || hostname.endsWith('.local')) throw new Error('Outbound localhost/local domains are blocked')
  if (!isAllowedHostname(hostname)) throw new Error('Outbound destination host is not in allowlist')
