...
export default function AdminConsolePage() {
  ...
  const [fetchingNowId, setFetchingNowId] = useState(null)
  ...
  async function fetchNow(account) {
    setFetchingNowId(account.id)
    try {
      const res = await fetch(`/api/admin/email-accounts/${account.id}/fetch-now`, { method: 'POST', headers: H })
      const d = await readJson(res)
      if (!res.ok) return flash(`Fetch failed: ${d.error}`, 'error')
      flash(`Fetched ${d.ingested} new email${d.ingested !== 1 ? 's' : ''} from "${account.account_name}".`, 'success')
    } catch {
      flash('Fetch failed — check server connection.', 'error')
    } finally {
      setFetchingNowId(null)
    }
  }
                      ...
                      <tr key={account.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                            ...
                            <button className="btn btn-danger" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => deleteEmailAccount(account)}>Delete</button>
                            {['Inbound', 'Both'].includes(account.direction) && (
                              <>
                                ...
                                <button className="btn btn-primary" style={{ fontSize: 11, padding: '3px 8px' }} disabled={fetchingNowId === account.id} onClick={() => fetchNow(account)}>
                                  {fetchingNowId === account.id ? 'Fetching...' : 'Fetch Now'}
                                </button>
                              </>
                            )}
